...
# Membangun model MDN-RNN di dalam kelas

class MDNRNN(object):

  # Inisialisasi semua parameter dan variabel kelas MDNRNN
  ...
  # Membuat metode yang membuat arsitektur model MDN-RNN itu sendiri
  def build_model(self, hps):
    ...
    # Melaksanakan operasi pelatihan
  ...
  # Membuat metode yang menginisialisasi sesi grafik tensorflow, digunakan untuk menjalankan inferensi atau pelatihan model MDN-RNN
  def _init_session(self):
    self.sess = tf.Session(graph=self.g)
    self.sess.run(self.init)

  # Membuat metode yang menutup sesi grafik tensorflow yang sedang berjalan (menutup sesi diperlukan untuk mengatasi grafik bersarang)
  def close_sess(self):
    self.sess.close()

  # Membuat metode yang mengekstraksi semua variabel yang dapat dilatih dari grafik RNN ke dalam daftar python
  ...
  # Membuat metode yang secara acak menginisialisasi parameter RNN
  ...
  # Membuat metode yang mengatur beberapa parameter menjadi nilai acak pada model RNN (biasanya dilakukan pada awal proses pelatihan)
  def set_random_params(self, stdev=0.5):
    rparam = self.get_random_model_params(stdev)
    self.set_model_params(rparam)

  # Membuat metode yang menetapkan bobot tertentu untuk nilai yang dipilih dalam model RNN
  ...
  # Membuat metode yang memuat bobot RNN yang disimpan dari file JSON
  def load_json(self, jsonfile='rnn.json'):
    with open(jsonfile, 'r') as f:
      params = json.load(f)
    self.set_model_params(params)

  # Membuat metode yang menyimpan bobot RNN terlatih ke dalam file JSON
...
# Membuat fungsi yang mengambil sampel indeks dari fungsi distribusi probabilitas (pdf)
...
# Membuat fungsi yang mengambil sampel urutan input untuk model RNN menggunakan model VAE yang telah dilatih sebelumnya
...
# Membuat fungsi yang mengembalikan state awal model RNN

def rnn_init_state(rnn):
  return rnn.sess.run(rnn.initial_state)

# Membuat fungsi yang mengembalikan state akhir model RNN
...
# Membuat fungsi yang mengembalikan ukuran output RNN tergantung pada modenya
...
# Membuat fungsi yang mengembalikan output RNN tergantung pada modenya
