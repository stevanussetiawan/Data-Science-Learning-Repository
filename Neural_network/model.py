# Menjalankan seluruh  Full World Model
...
# Membuat fungsi yang memuat dan mengembalikan model

def make_model(load_model=True):
  model = Model(load_model=load_model)
  return model

# Membuat fungsi kliping yang nantinya akan digunakan untuk memotong action

def clip(x, lo=0.0, hi=1.0):
  return np.minimum(np.maximum(x, lo), hi)

# Merakit seluruh Full World Model dalam satu kelas

class Model:

  # Inisialisasi semua parameter dan variabel kelas Model
  ...
  # Membuat metode yang menciptakan environment (dalam kasus kita, game CarRacing) di mana AI dan Manusia akan bermain
  ...
  # Membuat metode yang menginisiasi ulang status untuk model RNN
  def reset(self):
    self.state = rnn_init_state(self.rnn)

  # Membuat metode yang meng-encode pengamatan (frame input)
  ...
  # Membuat metode yang mengambil sampel action berdasarkan vektor laten z
  ...
  # Membuat metode yang menetapkan bobot yang diinisialisasi/dimuat ke dalam model
  ...
  # Membuat metode yang memuat bobot model
  ...
  # Membuat metode yang menginisialisasi bobot secara acak
  def get_random_model_params(self, stdev=0.1):
    return np.random.standard_cauchy(self.param_count)*stdev

  # Membuat metode yang menginisialisasi bobot secara acak untuk ketiga bagian Full World Model (VAE, RNN, Controller)
...
# Membuat fungsi yang menjalankan simulasi 1000 langkah Full World Model di dalam lingkungan CarRacing
...
# Mengimplementasikan kode utama untuk menjalankan kompetisi antara AI dan Manusia pada game CarRacing
