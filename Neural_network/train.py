# Melatih seluruh Full World Model
...
# Membuat fungsi yang menginisialisasi semua pengaturan pengoptimal yang berbeda
...
# Membuat fungsi yang mencetak informasi pelatihan dalam keluaran standar (fungsi ini dibuat karena kita menggunakan eksekusi paralel untuk setiap pekerja)

def sprint(*args):
  print(args)
  sys.stdout.flush()

# Membangun kelas yang bertindak sebagai seeder ke dalam proses pemilihan data untuk batch

class OldSeeder:

  # Inisialisasi semua parameter dan variabel kelas OldSeede
  def __init__(self, init_seed=0):
    self._seed = init_seed

  # Membuat metode yang mengembalikan benih berikutnya hanya dengan menambah benih saat ini
  def next_seed(self):
    result = self._seed
    self._seed += 1
    return result

  # Membuat metode yang menghasilkan id untuk batch berikutnya
...
class Seeder:
  ...
  # Menginisialisasi semua parameter dan variabel kelas Seeder
  def __init__(self, init_seed=0):
    np.random.seed(init_seed)
    self.limit = np.int32(2**31-1)

  # Membuat metode yang mengembalikan seed acak berikutnya
  def next_seed(self):
...
# Membuat fungsi yang meng-encode dan mengemas solusi dari worker ke dalam paket (paket adalah kumpulan data yang dikirim sebagai satu elemen, agar sesuai dengan format kerangka kerja MPI)
...
# Membuat fungsi yang meng-decode dan membongkar menerima paket solusi dari worker
...
# Membuat fungsi yang meng-encode dan mengemas hasil yang diperoleh worker ke dalam paket
...
# Membuat fungsi yang meng-decode dan membongkar paket yang diterima dari hasil yang diperoleh worker
...
# Membuat fungsi yang membuat worker dengan kumpulan bobot (parameter) tertentu untuk model, menjalankan simulasi dengannya, dan mengembalikan rata-rata reward setelah simulasi dimainkan
...
# Membuat fungsi yang membuat slave (slave adalah turunan dari model dengan solusi dan bobot spesifik) untuk model utama, mendapatkan kemungkinan solusi environment, membuat worker untuk setiap solusi, dan mengembalikan reward
...
# Membuat fungsi yang mengirimkan data pelatihan ke setiap slave
...
# Membuat fungsi yang menerima paket dari slave setelah slave selesai dengan proses simulasi dan mengumpulkan semua informasi (misalnya rata-rata reward)
...
# Membuat fungsi yang mengevaluasi sekumpulan states/frame dengan parameter model yang disediakan dan mengembalikan rata-rata reward
...
# Membuat fungsi yang mengimplementasikan seluruh proses pelatihan
...
# Membuat fungsi yang menjalankan pelatihan
...
# Membuat fungsi yang menciptakan environment pelatihan dengan parameter MPI
...
# Menerapkan kode utama yang meluncurkan seluruh pelatihan dengan serangkaian parameter yang dipilih
